...
var NewDefaultVaultOptions = vault.NewDefaultVaultOptions

type VaultOptions = vault.VaultOptions
...
func NewVaultRegistryStore(ctx context.Context, options *VaultOptions) (*VaultRegistryStore, error) {
	vaultcli, err := vault.NewVaultClient(ctx, options)
	if err != nil {
		return nil, err
	}
